...
gamefile = '/home/shaurya/develop/quackle/test/temp.gcg'
QUACKLELPLAYER = 1
...
def INVOKEprog(player, stringboard, thisrack, agentscore, opponentscore, secondstogo, errcode, moverequired, endofgameflag):
    ...
    if moverequired:
        starttime = time.time()
        if player==QUACKLELPLAYER:
            #run quackle
            #check format if suitable
            move = extension.quackle(gamefile,thisrack)
            print(str(move))
        else:
            #run other agent
            #eeshan will fill this up
            # move = raw_input(str(datatosend)+'\n') #player  'player' 's setting
            message = str(datatosend)
            cmd = "python greedy.py '"+ re.sub('\'', '\"', message) + "'" 
            print(cmd)

            move = commands.getoutput(cmd)
            print(str(move))
        endtime = time.time()
        timeconsumed = endtime - starttime
        return (move, timeconsumed)
...
def fullmove(player, showboard=False):
    ...
    # (7, 7, 'H', 'NOT', 'NOT', 4.132262945175171)
    ...
    if parsedmove[0] == 'pass':
        ...
        # >p2: INETTOD 8H NOT 3.18062710762 3.18062710762
        wordPlayed = parsedmove[4]
        move  = extension.translateMove(parsedmove[:3])
        moveFormatGcg = ' '.join(['>p'+str(player)+':', ''.join(rack[player]), move, wordPlayed, str(movescore),str(score[player])])
        #Update gcg file here
        extension.write2gcg(gamefile,moveFormatGcg)
    ...
    stringboard = ''.join([''.join(row) for row in letterboard])
    dictleftonrack = dict(Counter(rack[player]) - Counter(tilesplayed))
...
gameover = False
#prepare game file
extension.prepGcgHeader(gamefile)
while not gameover:
    #showall()
